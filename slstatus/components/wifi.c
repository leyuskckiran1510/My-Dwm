...
#define RSSI_TO_PERC(rssi) \
	...
	wifi_perc(const char *interface)
	{
		...
		if (!p || strcmp(status, "up\n") != 0) {
			if(!sent){
			fpp = popen("exec bash dunstify 'Wifi Disconnected' 'Conenction Lost' -u normal -t 1500 -r 15987 -i \
				~/Pictures/icons/no-wifi.png", "r");
			pclose(fpp);
			sent=1;
			sent2=0;
		}
			...
			if(!sent2){
			fpp = popen("exec bash nmcli connection show --active |grep -vi name |awk '{print$1}' | xargs -I{} dunstify 'Wifi Connected' 'Wifi Name:- {}' -u normal -t 1500 -r 15987 -i \
				~/Pictures/icons/wifi.png", "r");
			pclose(fpp);
			sent2=1;
			sent=0;
		}
